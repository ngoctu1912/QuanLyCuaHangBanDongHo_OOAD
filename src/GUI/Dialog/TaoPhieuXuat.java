...
public final class TaoPhieuXuat extends JPanel {
    ...
    private void initComponent(String type) {
        ...
        JPanel kJPanelBottom = new JPanel(new GridLayout(1, 2, 5, 0));
        kJPanelBottom.setOpaque(false);
        kJPanelBottom.setPreferredSize(new Dimension(0, 35));
        kJPanelBottom.setBorder(new EmptyBorder(0, 10, 0, 10));
        ButtonCustom btnKh = new ButtonCustom("Chọn khách hàng", "success", 14);
        ButtonCustom btnThemKh = new ButtonCustom("Thêm khách hàng", "success", 14);
        kJPanelBottom.add(btnKh);
        kJPanelBottom.add(btnThemKh);
        btnKh.addActionListener((ActionEvent e) -> {
            new ListKhachHang(TaoPhieuXuat.this, owner, "Chọn khách hàng", true);
        });
        btnThemKh.addActionListener((ActionEvent e) -> {
            new KhachHangDialog(null, owner, "Thêm khách hàng", true, "create");
        });
        
        JPanel khPanel = new JPanel(new GridLayout(3, 1, 5, 0));
        khPanel.setBackground(Color.WHITE);
        khPanel.setPreferredSize(new Dimension(0, 115));
    ...
    public boolean validateSelectDate(DTO.ChiTietMaKhuyenMaiDTO tmp) {
        ...
        // Nếu chưa đến thời gian bắt đầu -> không hợp lệ
        if (time_start != null && time_start.after(current_date)) {
            return true; // return true nghĩa là không hợp lệ (sẽ bị loại bỏ)
        }
        
        // Nếu đã quá thời gian kết thúc -> không hợp lệ
        if (time_end != null && time_end.before(current_date)) {
            return true; // return true nghĩa là không hợp lệ (sẽ bị loại bỏ)
        }
        
        // Nếu thời gian bắt đầu > thời gian kết thúc -> không hợp lệ
        if (time_start != null && time_end != null && time_start.after(time_end)) {
            return true; // return true nghĩa là không hợp lệ (sẽ bị loại bỏ)
        }
        
        return false; // return false nghĩa là hợp lệ (sẽ được thêm vào danh sách)
    }
